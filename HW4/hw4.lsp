...
; at-most-one-color in (1 2 3) is equivalent to
; at-most-one-color in (1 2) AND at-most-one-color in (1 3) AND at-most-one-color in (2 3)
; at-most-one-color in (x,y) is equivalent to "!x OR !y" by De-Morgan's Law
(defun at-most-one-color (n c k)
(cond	((= (- c 1) k) NIL)
	(t (append (at-most-cnf n c k (node2var n c k)) (at-most-one-color n (+ c 1) k))))	
)	
...
(generate-edge-cnf e 1 k)
  )

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; Helper functions:
(defun at-most-cnf (n c k start)
(cond 	((= c k) NIL)
	(t (append (list (list (- 0 start) (- 0 (node2var n (+ c 1) k)))) (at-most-cnf n (+ c 1) k start))))
)

(defun generate-edge-cnf (e c k)
(cond	((= (- c 1) k) NIL)
	(t (append (list (list (- 0 (node2var (car e) c k)) (- 0 (node2var (cadr e) c k)))) (generate-edge-cnf e (+ c 1) k))))
)
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
